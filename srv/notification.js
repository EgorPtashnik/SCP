module.exports = srv => {
  const ENTITY_NOTIFICATION = "notification_Notification";

  srv.on("GetBadgeNumber", req =>
    cds.tx(req)
       .run(`SELECT * from ${ENTITY_NOTIFICATION} where NotificationCount = 1`)
       .then(res => res.length)
  );
  srv.on("GetBadgeNumberByIntent", async req => {
    const { NavigationIntent } = req.data;
    return await cds.tx(req)
                    .run(`SELECT * from ${ENTITY_NOTIFICATION} where NotificationCount = 1 and NavigationIntent = ${NavigationIntent}`)
                    .length;
  });

  srv.on("ResetBadgeNumber", req =>
    cds.tx(req).run(
      UPDATE(ENTITY_NOTIFICATION)
        .set({NotificationCount: 0})
        .where( {NavigationIntent: { "=": NavigationIntent}} )
    )
  );
  ...
  srv.on("Dismiss", req => {
    ...
    cds.tx(req).run(
      DELETE.from(ENTITY_NOTIFICATION)
